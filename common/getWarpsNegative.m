...
%Requires fAPI5 to be prerun to set h and w

global local;
local=0;
name='atkinson';
f.l=['W:\Fintan\Data\motion\processed\' name '\oval-centroid-120\'];
f.r=['\\komodo\SharedData\Fintan\Data\motion\processed\' name '\oval-centroid-120\'];

checkDir([f.l 'warp\warpPos']);
checkDir([f.l 'warp\warpNeg']);
...
for i=1:nOps
    ...
    if lastOpCompleted < nOps
        if local
            task_warp_group_util_neg(h,w,frameFilesSlice,refImage,dirFlowFields.l,firstOp:finalOp);
        else
        tasks{i}=createTask(job,@task_warp_group_util_neg,2,...
