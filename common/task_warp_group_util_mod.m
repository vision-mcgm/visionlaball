function [ ok,tFin ] = task_warp_group_util_mod(h,w,frameFileSlice, refImageSlice, dirFlowFields,nSlice)

%Modified to use only the full-scale McGM and not interpolate with
...
for op=1:nOps
    ...
    ref=double(imread(refImageSlice(op).name));
    ...
    target2=conv2(conv2(rgb2gray(target/255), G, 'same'), G', 'same'); %Apply Gaussian filter by convolution
    ...
    rec2=conv2(conv2(rgb2gray(rec/255), G, 'same'), G', 'same'); %Apply the Gaussian filter by convolution
    [shot vel angles]=mcgm2(rec2, target2, mcgmcpp, mcgmconv, mcgmthresh);
    [Ux Uy]=out2vecs(vel, angles);
    Ux(isnan(Ux))=0;
    Uy(isnan(Uy))=0;
    singleX=Ux;
    singleY=Uy; %Save the warp fields from the Gaussian filtered comparison
    ...
    TWy = prevWy;
    TWx=Ux;
    TWy=Uy; %Save the unprocessed fields.
    save([dirFlowFields 'warp' num2str(n,'%03d') '.mat'],'TWx','TWy');
    %save([dirFlowFields 'warp' num2str(n,'%03d') '.mat'],'Ux','Uy'); %Save the direct fields
