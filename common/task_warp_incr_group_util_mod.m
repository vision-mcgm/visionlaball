function [ ok,tFin ] = task_warp_incr_group_util_mod(h,w,frameFileSlice, refImageSlice, dirFlowFields,nSlice)

%Doesn't compare across multiple scales - simply does one McGM.
...
%nSlice is an interval with the slice numbers for this slice of work.
  ...
  % altixLog('Im doing it 200.');
	...
	TWy = prevWy;
    TWx=singleX;
    TWy=singleY; %Save the single warp fields instead of the combined ones
	save([dirFlowFields 'warp' num2str(n,'%03d') '.mat'],'TWx','TWy');
    	%save([dirFlowFields 'warp' num2str(n,'%03d') '.mat'],'singleX','singleY');
