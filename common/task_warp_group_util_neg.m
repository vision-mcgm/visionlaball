function [ ok,tFin ] = task_warp_group_util_neg(h,w,frameFileSlice, refImage, dirFlowFields,nSlice)


%MODIFIED to also warp on the negative image, then interpolate the results.

%FrameFileSlice is a struct array like frameFileSlice(i).name.
%refImage is a string.
%dirFlowFields is a string
%nSlice is an interval with the slice numbers for this slice of work.
...
for op=1:nOps
    ...
    for neg=1:2 %Loop over positive and negative
    ...
    target=target/255;
        if neg==2
            ...
            target=imcomplement(target);
        end
        ref2=conv2(conv2(rgb2gray(ref),G,'same'), G','same');
        ...
        target2=conv2(conv2(rgb2gray(target), G, 'same'), G', 'same');
        ...
        rec2=conv2(conv2(rgb2gray(rec), G, 'same'), G', 'same');
    ...
    end %End negative loop
    TWx = wX{1}; %Connect these correctly
    TWy = wY{1};
    save([dirFlowFields 'warpPos\warp' num2str(n,'%03d') '.mat'],'TWx','TWy');
    
    TWx = wX{2}; %Connect these correctly
    TWy = wY{2};
    save([dirFlowFields 'warpNeg\warp' num2str(n,'%03d') '.mat'],'TWx','TWy');
    
    
    TWx = (wX{1}+wX{2})/2; %Connect these correctly
    TWy = (wY{1}+wY{2})/2;
