function [  ] = buildPCA_Marion( c ,speechvector)
...
if ~success
    ...
    if(~checkSuccess(tasks_3,allresults,2));
        ...
        if ~success
            ...
            thisallDir=[remPath(c,c.dirPCAModel) 'all\'];
            thismorphDir=[remPath(c,c.dirPCAModel) 'morph\'];
            tasks_5{1}=createTask(job,@task_collectMorphVectors_Marion_4,nWarpOutArgs,{c,thismorphDir,thisallDir,speechvector});
            tic;									% Start a timer.
            submit(job);
            waitForState(job);
            tasks_5{1}
            ...
            if(~checkSuccess(tasks_5,allresults,4));
                ...
                tasks_6{1}=createTask(job,@task_PCA_Marion,nWarpOutArgs,{c});
                else
                    task_PCA(c);
                end
                tic;									% Start a timer.
