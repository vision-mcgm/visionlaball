...
switch1=1; %Warps
switch2=1; %Find mean warp
switch3=1; %Express as mean-rel
switch4=1; %Collect morph vectors
switch5=1; %PCA
...
if switch1
    ...
    if not(local)
        ...
        if sum(nonempty)
            success=false;
        end% Display any non-empty error messages of tasks in the job object.
    end
...
if ~success
    fprintf( 'Not doing job 2');
   % statusMsg('1','NOT','Warps');
   %We're not guaranteed to have done the warps (they may have been
   %switched off) so can't say they're successful
elseif switch2
   % statusMsg('1','','Warps');
