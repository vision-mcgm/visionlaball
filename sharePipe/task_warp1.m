function [ ok,tFin ] = task_warp1(frameFile, refImage, dirFlowFields,n,wrkDir)
	%Warp images
	tStart=tic;
...
%{
	% Record details of where the task has been run, in a file.
	i=1;
	cFin=1;
	datFile=[dirFlowFields 'warp' num2str(n) '.mat'];		% The output data file name.
	wkrname=get(getCurrentWorker,'Name');			% Get the name of the current worker.
	tFin=toc(tStart);
	fname=['task2_' num2str(n) '_' wkrname '.txt'];			% Create a unique filename, using the worker name and task number	
	fid=fopen(fname,'w');
	fprintf(fid,'Worker name: %s\nWorking directory: %s\nStatus file: %s\nOutput data file: %s\n',wkrname,pwd,fname,datFile);
	fprintf(fid,'\nFrames completed: %i\nTotal retries: %i\nTotal time taken: %10.6f secs\n',i,cFin,tFin);
	fclose(fid);
    	c=60;						% Set the iteration counter for how long to wait.
	while c>0					% Check the time limit.
		if exist(dirFlowFields,'file')==7		% Test whether the folder exists yet.
...
function [Wx, Wy] = cswarp(ref, target, scales, mcgmcpp, mcgmconv, mcgmthresh, nIts)
	...
	for i = 1:size(scales, 2),
		srec = imresize(rec, scales(i)*[h w], 'bilinear');
		starget = imresize(target, scales(i)*[h w], 'bilinear');
