function [ im_out ] = wipeOval( im,x,y,w,h )
%This is built to take uint8s
...
im_out=uint8(zeros(height,width,3));
...
if pointDist > borderDist
    out=[0,0,0];
elseif borderDist-pointDist < fadeOut
    %disp(fadeOut-(borderDist-pointDist)*10);
    %point(1,1,2)=255;
    out=point-(((pointDist-borderDist)+fadeOut)*25.5);
    %out=point - (fadeOut-(borderDist-pointDist))*10;
    %out=point;
else
    out=point;
end
