...
switchKeypoints=1;
switchProject=1;
switchTransform2=1;  %This does moving as well
switchAveraging=1;
AveragingLocal=0; %Whether to do the averaging locally or on the cluster
switchExtremes=1;
switchPCA2=0;
...
prec='%03d'; %Precision of numericals in filenames. Increase if we will ever have over a thousand files.
    ...
    for Ms=1:models
        ...
        for Mt=1:models
            ...
            for e=1:nFramesInSource(Ms)
                ...
                prelim_transOutPath=[root expDirs{Ms,e} 'trans\frame' num2str(e,prec) '_model' num2str(Mt,prec) '.bmp'];
...
if switchExtremes
    ...
    for Ms=1:models
        for Mt=1:models
            for e=1:nFramesInSource(Ms)
                %This loop core corresponds to one expression. Let's PCA
                %it.
                cTemplate=config('AVATAR_PCA_TEMPLATE_FOR_AUTOGEN.pca');
                cThisModel=cTemplate;
                cThisModel.h=c.h;
                ...
                thisSourceDir=[c.dirOutput 'exps\m' num2str(Ms) 'exp' num2str(e) '\trans\'];
                sourceDirList=dir([thisSourceDir '*.bmp']);
               
                cThisModel.referenceFrameFiles{1}=sourceDirList(c.referenceModel).name;
                %modelConfigs{c.referenceModel}.referenceFrameFiles;
                cThisModel.dirSourceBitmaps=thisSourceDir; %This path needs to be rel to the root
                cThisModel.clusterRoot=c.clusterRoot;
                cThisModel.localRoot=c.localRoot;
                cThisModel.dirPCAModel = [thisSourceDir 'pca\']; %This path needs to be rel to the root
                
                cThisModel.PCs=models;
                cThisModel=rmfield(cThisModel, 'frames');
                
                save(rightPath(c,[thisSourceDir 'config.mat']),'cThisModel');
...
%----------------------------------BEGIN averaging code - this is always
%switched off here as we're doing PCA instead
if switchAveraging
