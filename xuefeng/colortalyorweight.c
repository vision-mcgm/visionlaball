...
void colortalyorweight(double *out, double *orders, double *zone, double *offset)
{
    int ix, iy, it, is, px, py, pt, ps, p;
    int x_orders, y_orders, t_orders, s_orders, x_zone, y_zone, t_zone, s_zone;
	float mult1, mult2, mult3, x, y, t, s, weight, timeoffset;
    
	x_zone = (int) zone[0];	y_zone = (int) zone[1];	t_zone = (int) zone[2]; 
    s_zone = (int) zone[3];  
    x_orders = (int) orders[0];	y_orders = (int) orders[1];	t_orders = (int) orders[2];
    s_orders = (int) orders[3];
    ...
    p = 0;
    for (ps = 0; ps < s_orders; ps++){
        for (pt = 0; pt < t_orders; pt++){
            for (py = 0; py < y_orders; py++){
                 for (px = 0; px < x_orders; px++){
                    out[p] = 0.0;
                    for (is = 0; is < s_zone; is++){
                        s = (float) ((float)is - ((float) s_zone - 1.0)/2.0);
                        for (it = 0; it < t_zone; it++){
                            ...
                            for (iy = 0; iy < y_zone; iy++){
                                ...
                                for (ix = 0; ix < x_zone; ix++){
                                    ...
                                    weight = (trap_pow( x/mult1, (float) px) * trap_pow( y/mult2, (float) py)* trap_pow( mult3*t, (float) pt) * trap_pow( mult3*s, (float) ps))
                                    / (float)(fac(px) * fac(py) * fac(pt) * fac(ps));
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    plhs[0] = mxCreateDoubleMatrix((int)orders[0] * (int)orders[1] * (int)orders[2] * (int)orders[3], 1, mxREAL);
  
    out = mxGetPr(plhs[0]);
    colortalyorweight(out, orders, zone, offset);
}